...
- **상품 데이터**: 772개 상품 (기존 93개에서 대폭 확장)
- **리뷰 데이터**: 약 7,000개 리뷰 (프로필, 평점, 상세 텍스트 포함)
- **이미지 데이터**: 제품별 상세/연출 이미지 (2GB+)
- **고객 데이터**: 277명 (기존 가상 고객 프로필 유지)
...
├── data/
├── data/
│   ├── products.json          # [NEW] 772개 통합 제품 데이터 (카테고리, 이미지 경로 포함)
│   ├── images/                # [NEW] 제품별 고화질 이미지 리포지토리
│   ├── customers.json         # 277명 고객 프로필
│   └── amoremall_reviews.json # (Legacy) 초기 소규모 데이터셋
├── model.pth                   # 학습된 가중치
